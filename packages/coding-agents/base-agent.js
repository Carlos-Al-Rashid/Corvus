 ...
 * - Logging
 */
import { AgentError, EscalationError, } from './types/index';
...
export class BaseAgent {
    ...
    async run(task) {
        ...
        try {
            ...
            if (this.traceLogger) {
                this.traceLogger.endAgentExecution(this.agentType, 'failed', undefined, error.message);
            }
            ...
            const durationMs = Date.now() - this.startTime;
            globalMetricsCollector.onAgentFailed(this.agentType, task.id, error.message, durationMs);
...
`;
     ...
     */
    async sendMessage(to, type, payload, priority = MessagePriority.MEDIUM) {
        const message = globalMessageBus.createMessage(this.agentType, to, type, payload, priority);
        ...
        logger.system(`Message sent: ${this.agentType} → ${to} [${type}] - ${response.success ? 'success' : 'failed'}`);
        return response;
    }
    /**
    ...
    async receiveMessage(message) {
        logger.system(`${this.agentType} received message from ${message.from}: ${message.type}`);
        // Log message if trace logger available
        if (this.traceLogger) {
            this.traceLogger.recordAgentMessage(message);
        }
