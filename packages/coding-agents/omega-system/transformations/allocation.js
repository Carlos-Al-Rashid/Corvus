 ...
 */
function findBestAgent(task, capabilities) {
    ...
    const compatibleAgents = capabilities.filter(c => c.supportedTaskTypes.includes(task.type));
 ...
 */
function createWorkerAssignments(tasks, capabilities, maxWorkers) {
    const assignments = [];
    ...
    for (const [agentType, agentTasks] of agentTaskMap) {
        ...
        const workersNeeded = Math.min(Math.ceil(agentTasks.length / 3), // Max 3 tasks per worker
        maxConcurrency, maxWorkers - assignments.length);
        for (let w = 0; w < workersNeeded; w++) {
            ...
            if (workerTasks.length > 0) {
                const assignment = {
                    ...
                    estimatedDurationMs: workerTasks.reduce((sum, t) => sum + (t.estimatedDuration || 0) * 60000, 0),
                    estimatedTokens: workerTasks.reduce((sum, t) => sum + t.estimatedTokens, 0),
 ...
 */
function createExecutionBatches(taskSet, workers) {
    const batches = [];
    const taskToWorker = new Map();
    ...
    for (let level = 0; level < taskSet.dag.levels.length; level++) {
        ...
        const maxDuration = Math.max(...batchWorkers.map(w => w.estimatedDurationMs), 0);
        const batch = {
 ...
 */
function calculateUtilization(workers, capabilities) {
    const byAgent = {};
 ...
 */
function calculateOptimization(workers, batches, taskSet) {
    ...
    const variance = taskCounts.reduce((sum, count) => sum + Math.pow(count - avgTasks, 2), 0) / taskCounts.length || 0;
 ...
 * ```
 */
export async function allocation(taskSet, world) {
    ...
    const workers = createWorkerAssignments(taskSet.tasks, AGENT_CAPABILITIES, maxWorkers);
...
export function validateAllocation(alloc) {
    ...
    const duplicates = allTasks.filter((t, i) => allTasks.indexOf(t) !== i);
