 ...
 */
async function executeTask(taskId, workerId, agentType) {
 ...
 */
async function executeWorker(worker, parallel = false) {
    const taskResults = [];
    const errors = [];
    if (parallel) {
        // Execute all tasks in parallel
        const results = await Promise.all(worker.assignedTasks.map(taskId => executeTask(taskId, worker.workerId, worker.agentType)));
 ...
 * ```
 */
export async function execution(allocation) {
    const startTime = Date.now();
    ...
    const allTaskResults = batchResults.flatMap(br => br.workerResults.flatMap(wr => wr.taskResults));
    ...
    const throughput = totalDuration > 0
        ...
        : 0;
    const parallelBatches = batchResults.filter(br => br.workerResults.length > 1).length;
    const resultSet = {
 ...
 */
export function validateResultSet(results) {
    const errors = [];
 ...
 * Retry failed tasks
 */
export async function retryFailedTasks(results, allocation) {
    ...
    for (const taskId of failedTaskIds) {
        const worker = allocation.workers.find(w => w.assignedTasks.includes(taskId));
        if (worker) {
            const result = await executeTask(taskId, worker.workerId, worker.agentType);
