 ...
 */
function goalToObjective(goal, _source) {
    return {
        id: `obj-${goal.id}`,
 ...
 */
function inferCapabilities(goal) {
    const capabilities = [];
    const desc = goal.description.toLowerCase();
 ...
 */
function assessRisks(_intent, world, objectives) {
    const factors = [];
    const blockers = [];
    const dependencies = [];
    // Check complexity risk
    const complexObjectives = objectives.filter(o => o.estimatedComplexity === 'complex' || o.estimatedComplexity === 'very-complex');
 ...
 */
function determineStrategy(objectives, resources, risks) {
    ...
    if (approach === 'sequential') {
        ...
        const simple = objectives.filter(o => o.estimatedComplexity === 'trivial' || o.estimatedComplexity === 'simple');
        const complex = objectives.filter(o => o.estimatedComplexity !== 'trivial' && o.estimatedComplexity !== 'simple');
        if (simple.length > 0) {
            phases.push({
                phaseId: 'phase-1',
 ...
 * ```
 */
export async function understanding(intent, world) {
