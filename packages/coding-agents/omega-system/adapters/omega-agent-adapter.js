...
export class OmegaAgentAdapter {
    ...
    async execute(request) {
        ...
        try {
            ...
            const errorReport = this.createErrorResponse(error, request);
            return {
                success: false,
                report: errorReport,
                durationMs: Date.now() - startTime,
     ...
     */
    enhanceIntentForAgent(intent, agentType) {
 ...
 */
export function createOmegaAdapter(config) {
    return new OmegaAgentAdapter(config);
}
/**
 * Execute a single agent request (convenience function)
 */
export async function executeWithOmega(request) {
    const adapter = new OmegaAgentAdapter();
    return adapter.execute(request);
}
//# sourceMappingURL=omega-agent-adapter.js.map