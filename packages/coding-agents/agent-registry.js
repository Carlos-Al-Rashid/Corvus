...
export class AgentRegistry {
    ...
    analyzer;
    toolFactory;
    assignments = new Map(); // taskId -> agent
    analysisCache; // taskId -> analysis with TTL
    config;
    defaultHookManager;
    constructor(config) {
     ...
     */
    setDefaultHookManager(hookManager) {
    ...
    async assignAgent(criteria) {
        ...
        try {
            ...
            logger.info(`  Complexity: ${analysis.complexity.category} (${analysis.complexity.complexityScore}/100)`);
            ...
            if (analysis.assignmentStrategy.type === 'reuse-existing' &&
                ...
                if (existingAgent && existingAgent.canHandleTask(task)) {
                    ...
                    if (!maxConcurrentTasks || runningTasks < maxConcurrentTasks) {
                        ...
                        if (createdHooks.length > 0) {
                            existingAgent.hookManager = hookManager;
                        }
                        this.assignments.set(task.id, existingAgent);
                        logger.success(`✓ Assigned existing agent: ${existingAgent.getInstanceId()}`);
                        logger.info(`  Created ${createdTools.length} tools, ${createdHooks.length} hooks`);
                        return {
                            success: true,
                        ...
                        logger.info(`Agent ${existingAgent.getInstanceId()} has reached max concurrent tasks (${maxConcurrentTasks})`);
                    }
                }
            }
            ...
            if (analysis.capabilityAnalysis.recommendation === 'use-existing') {
                ...
                logger.info(`  Using existing template: ${analysis.capabilityAnalysis.matchingTemplates[0].template.name}`);
            }
            else if (analysis.capabilityAnalysis.recommendation === 'extend-existing') {
                templateId = analysis.capabilityAnalysis.matchingTemplates[0].template.id;
                logger.info(`  Extending template: ${analysis.capabilityAnalysis.matchingTemplates[0].template.name}`);
                logger.info(`  Adding capabilities: ${analysis.capabilityAnalysis.requiredNewCapabilities.join(', ')}`);
            }
            else {
     ...
     */
    getAgentRunningTaskCount(agent) {
