...
export class ToolFactory {
     ...
     */
    generateCommandTool(requirement) {
...
`.trim();
     ...
     */
    generateLibraryTool(requirement) {
        const { name } = requirement;
        return `
export async function ${this.sanitizeName(name)}(params: any): Promise<any> {
  // Library tool: ${name}
...
`.trim();
     ...
     */
    generateServiceTool(requirement) {
        const { name, parameters } = requirement;
        return `
...
`.trim();
     ...
     */
    getImplementationType(toolType) {
        switch (toolType) {
            case 'command':
    ...
    async exportTool(toolId, outputPath) {
        ...
        try {
            ...
            logger.error(`Failed to export tool: ${error.message}`);
            return false;
        }
    }
}
