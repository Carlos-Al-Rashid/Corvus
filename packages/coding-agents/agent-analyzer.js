...
export class AgentAnalyzer {
     ...
     */
    async analyzeTask(task, availableTemplates) {
        ...
        const capabilityAnalysis = await this.analyzeCapabilities(requirements, availableTemplates);
        // Step 4: Determine assignment strategy
        const assignmentStrategy = this.determineAssignmentStrategy(complexity, capabilityAnalysis);
        const result = {
            complexity,
            requirements,
            capabilityAnalysis,
        ...
        logger.success(`✓ Analysis complete: ${assignmentStrategy.type} (confidence: ${assignmentStrategy.confidence}%)`);
        return result;
    }
    /**
    ...
    async analyzeComplexity(task) {
        ...
        const estimatedEffort = Math.max(1, Math.round(complexityScore / 20) * requiredCapabilities.length);
        return {
     ...
     */
    async determineRequirements(task, complexity) {
        const tools = [];
        const hooks = [];
        const templates = [];
        // Determine execution strategy
        const strategy = complexity.complexityScore > 70 ? 'conditional' : 'sequential';
        // Add required tools
     ...
     */
    async analyzeCapabilities(requirements, availableTemplates) {
        ...
        for (const template of availableTemplates) {
            ...
            const missingCapabilities = requirements.capabilities.filter((cap) => !templateCapabilities.includes(cap));
            const matchScore = Math.round(((requirements.capabilities.length - missingCapabilities.length) /
     ...
     */
    determineAssignmentStrategy(complexity, capabilityAnalysis) {
        let type;
        let reason;
        let confidence;
        ...
        // Fallback strategy
        const fallback = type === 'reuse-existing'
            ? {
                type: 'create-new',
    ...
    inferToolType(toolName) {
        ...
        if (commandTools.includes(toolName))
            return 'command';
        if (apiTools.includes(toolName))
            return 'api';
        if (serviceTools.includes(toolName))
            return 'service';
        return 'library';
    }
}
